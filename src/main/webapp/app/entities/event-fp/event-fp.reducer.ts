...
import { IEventFp, defaultValue } from 'app/shared/model/event-fp.model';
...
const initialState = {
  loading: false,
  errorMessage: null,
  entities: [] as ReadonlyArray<IEventFp>,
  entity: defaultValue,
  updating: false,
  updateSuccess: false
};

export type EventFpState = Readonly<typeof initialState>;

// Reducer

export default (state: EventFpState = initialState, action): EventFpState => {
  switch (action.type) {
...
export const getEntities: ICrudGetAllAction<IEventFp> = (page, size, sort) => ({
  type: ACTION_TYPES.FETCH_EVENT_LIST,
  payload: axios.get<IEventFp>(`${apiUrl}?cacheBuster=${new Date().getTime()}`)
});

export const getEntity: ICrudGetAction<IEventFp> = id => {
  const requestUrl = `${apiUrl}/${id}`;
  return {
    type: ACTION_TYPES.FETCH_EVENT,
    payload: axios.get<IEventFp>(requestUrl)
  };
};

export const createEntity: ICrudPutAction<IEventFp> = entity => async dispatch => {
...
export const updateEntity: ICrudPutAction<IEventFp> = entity => async dispatch => {
  const result = await dispatch({
...
export const deleteEntity: ICrudDeleteAction<IEventFp> = id => async dispatch => {
  const requestUrl = `${apiUrl}/${id}`;
  const result = await dispatch({
